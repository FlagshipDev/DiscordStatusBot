...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Despiadados' : '⚫ DespiadadosCommunity')
    .setTimestamp(new Date())
    //.addField('\n\u200b\nBienvenido al canal de estado del servidor','En este canal podrás comprobar el estado del servidor sin necesidad de preguntar.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Estado del servidor',':white_check_mark: Online',true)
        .addField('Cola',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Jugadores en línea',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true)
        .addField('\u200b','\u200b\n\u200b\n',true);
        //if (players.length > 0) {
          // method D
          //const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('LeyendasRP',{'url':'https://www.twitch.tv/theflagship','type':'STREAMING'});
